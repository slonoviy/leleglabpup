...
app.config['RECAPTCHA_PUBLIC_KEY'] = '6LeSKD8mAAAAALNwk15ctdlMsLHoQPrJhLTtDo-V'
app.config['RECAPTCHA_PRIVATE_KEY'] = '6LeSKD8mAAAAAPZABMyviBcTwpd02IemRBLgl418'
...
def swap_and_save(image_path, file_name,image_folder):
    ...
    half_width = width // 2
    top_left = np_img[:half_height, :half_width]
    top_right = np_img[:half_height, half_width:]
    bottom_left = np_img[half_height:, :half_width]
    bottom_right = np_img[half_height:, half_width:]

    top_left.save(f"{image_folder}/left_up_{file_name}")
    top_right.save(f"{image_folder}/Right_up_{file_name}")
    bottom_left(f"{image_folder}/left_down_{file_name}")
    bottom_right(f"{image_folder}/Right_down_{file_name}")
